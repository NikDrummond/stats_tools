...
def compare_groups(
    ...
    alternative: _Alternative = "two_sided",
    ...
    # 2. extract arrays
    groups_jax, data_jax, metadata = _df_to_jax(
        df_clean, group_col=group, value_col=measure
    )
    ...
    if method == "parametric":
        if k == 2:
            ...
            eff = (
                robust_eta_squared(groups_jax, data_jax)
                if robust
                else eta_squared(groups_jax, data_jax)
            )
    elif method == "rank":
        ...
        stat_fn = (
            ...
            if k == 2 and method == "parametric"
            else (lambda d, g: _one_way_anova(g, d)[0])
        )
        perm_stats = _null_distribution(
            stat_fn, data_jax, groups_jax, n_perm=n_permutations, seed=random_state or 0
        )
...
def regress_with_groups(
    ...
    X_pd = pd.DataFrame(
        {"__intercept": 1.0, predictor: df_clean[predictor].astype(float)}
    )
    if group:
    ...
    if method == "permutation" and n_permutations > 0:
        ...
        perm_stats = _null_distribution(
            stat_fn, X, y, n_perm=n_permutations, seed=random_state or 0
        )
        p_val = empirical_p_value(
            stat_fn, X, y, n_perm=n_permutations, seed=random_state or 0
        )
