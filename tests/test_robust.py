...
def test_huber_m_estimator_no_outliers():
    ...
    # should be close to sample median (allow ±0.01)
    assert abs(float(mu) - np.median(x)) < 0.01
    # sigma is approx MAD/0.6745
    mad = np.median(np.abs(x - np.median(x)))
    expected_sigma = mad / 0.6745
    assert abs(float(sigma) - expected_sigma) < 0.1
...
def test_robust_se_huber_consistency():
    ...
    assert se_robust == pytest.approx(se_classic, rel=0.2)
