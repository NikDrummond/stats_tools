...
def test_cohens_d_known_value():
    ...
    # compute expected via the same formula:
    # pooled SD and Hedges’ J correction
    a_np, b_np = np.array(a), np.array(b)
    n1, n2 = len(a_np), len(b_np)
    var1 = np.var(a_np, ddof=1)
    var2 = np.var(b_np, ddof=1)
    pooled_var = ((n1-1)*var1 + (n2-1)*var2)/(n1+n2-2)
    pooled_sd = np.sqrt(pooled_var)
    raw_d = (a_np.mean() - b_np.mean())/pooled_sd
    df = n1 + n2 - 2
    J = 1 - (3.0/(4.0*df - 1.0))
    expected = raw_d * J
