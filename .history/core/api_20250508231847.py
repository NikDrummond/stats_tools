...
def compare_groups(
    ...
    if method == "parametric":
        if k == 2:
            ...
            stat, p_val = _t_test(x1, x2, equal_var=not robust)
            eff = (robust and robust_cohens_d(x1, x2)) or cohens_d(x1, x2)
        else:
            stat, p_val = _one_way_anova(groups_jax, data_jax)
            eff = (robust and robust_eta_squared(groups_jax, data_jax)) or eta_squared(groups_jax, data_jax)
    elif method == "rank":
        stat, p_val = _kruskal_wallis(groups_jax, data_jax)
        if k == 2:
            x1, x2 = data_jax[groups_jax == 0], data_jax[groups_jax == 1]
            eff = rank_biserial(x1, x2)
        else:
...
def regress_with_groups(
    ...
    """
    Fit linear model, optionally testing group or group×predictor interaction.
    
    :param df: DataFrame in long form.
    :param outcome: name of outcome column.
    :param predictor: name of numeric predictor.
    :param group: optional group factor for main/interactions.
    :param method: only "parametric" or "permutation" supported.
    :param robust: currently ignored (future Huber/trimmed regression).
    :param n_permutations: for permutation tests of predictor term.
    :param p_adjust: not currently used (no multiple tests by default).
    :param ci: whether to compute CI around β or R².
    :param ci_method: "analytic", "bootstrap", or "permutation".
    :param random_state: seed.
    :return: RegressionTestResult.
    :raises ValueError: for missing columns or invalid method.
    """
    # 1. validate & drop NA
    ...
    # intercept + predictor [+ group dummies + interaction if group]
    X_pd = df_clean[[predictor]].copy()
    X_pd["__intercept"] = 1.0
    if group:
        levels = pd.Categorical(df_clean[group]).categories
        for lvl in levels[1:]:
            X_pd[f"g_{lvl}"] = (df_clean[group] == lvl).astype(float)
        # interaction terms
        for lvl in levels[1:]:
            X_pd[f"ix_{lvl}"] = X_pd[predictor] * X_pd[f"g_{lvl}"]
    X = jnp.asarray(X_pd.values)
    y = jnp.asarray(df_clean[outcome].values)
    # 3. fit
    betas, se, t_stats, p_vals = _linear_regression(X, y)
    # predictor is column 1
    coef = float(betas[1])
    se_coef = float(se[1])
    t_stat = float(t_stats[1])
    p_val = float(p_vals[1])
    # 4. effect size: standardized β
    eff = float(rank_biserial(X[:,1], y)) if method=="rank" else float(
        jnp.sum(((X[:,1]-X[:,1].mean())/X[:,1].std(ddof=1)) *
                ((y-y.mean())/y.std(ddof=1))) / (y.size - 1)
    )
    # 5. permutation p-value if requested
    perm_stats = None
    if method == "permutation" and n_permutations > 0:
        stat_fn = lambda Xmat, yvec: float(
            _linear_regression(Xmat, yvec)[2][1]  # t_stat
        )
    ...
    if ci:
        if ci_method == "analytic":
            ci_lower, ci_upper = analytic_ci(coef, se_coef, df=y.size - X.shape[1])
        elif ci_method == "bootstrap":
            ci_lower, ci_upper = bootstrap_ci(
