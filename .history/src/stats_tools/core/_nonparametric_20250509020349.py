...
def _rank_data(data: jnp.ndarray) -> jnp.ndarray:
    ...
    """
    # Note: this does *not* average ties.
    import numpy as onp
    arr = onp.array(data)
    # competition rank via double argsort
    ranks = onp.argsort(onp.argsort(arr)) + 1
...
def _kruskal_wallis(
    ...
    # 2) determine number of groups k = max(labels)+1
    k = jnp.max(labels).astype(int) + 1

    # 3) sum term via fori_loop
    ...
    # 5) χ² survival function (upper‐tail): df = k-1
    df = k - 1
    # regularized incomplete beta argument z = df/(df + H)
    z = df / (df + H)
    # CDF = I_{df/2, 1/2}(z); survival = 1 - CDF
    cdf = spec.betainc(df / 2.0, 0.5, z)
    p   = 1.0 - cdf
