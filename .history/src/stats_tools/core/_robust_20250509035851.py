...
from ._effect_sizes import eta_squared
...
def robust_eta_squared(
    ...
    if proportion_to_cut == 0.0:
        from ._effect_sizes import eta_squared
        return float(eta_squared(groups, data))

    # compute overall trimmed mean
    overall_trim = float(trimmed_mean(data, proportion_to_cut))

    # SS_between via Python loop & boolean indexing
    ss_between = 0.0
    for level in np.unique(np.array(groups)):
        mask = groups == level
        grp_data = data[mask]
        m_trim = float(trimmed_mean(grp_data, proportion_to_cut))
        ss_between += grp_data.size * (m_trim - overall_trim) ** 2

    ss_total = float(jnp.sum((data - overall_trim) ** 2))
    # if total sum of squares is zero, all data identical → η² = 0
    if ss_total == 0.0:
        return 0.0
...
+    _, sigma = huber_m_estimator(x, c)+    n = x.size
+    return sigma / jnp.sqrt(n)
