...
def _one_way_anova(
    ...
    # F-statistic
    F_naive = msb / msw
    F = jnp.where(ss_within > 0, F_naive, 0.0)

    # p-value via the upper‐tail of the F distribution
    z       = (df_between * F) / (df_between * F + df_within)
    cdf_naive = spec.betainc(df_between / 2.0, df_within / 2.0, z)
    p_naive   = 1.0 - cdf_naive
    # if within‐SS zero, set p=1
    p = jnp.where(ss_within > 0, p_naive, 1.0)
