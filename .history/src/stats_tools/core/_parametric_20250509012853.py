...
def _one_way_anova(
    ...
    """
    One‐way ANOVA without dynamic unique.
    """
    n = measure.size
    ...
    # between‐groups SS via fori_loop calling our top‐level body
    ss_between = lax.fori_loop(
        0,
        k,
        lambda idx, acc: _anova_ssb_body(idx, acc, labels, measure, grand_mean),
        0.0
    )
    ...
    cdf = spec.betainc(df_between/2, df_within/2, z)
    p   = 1.0 - cdf
