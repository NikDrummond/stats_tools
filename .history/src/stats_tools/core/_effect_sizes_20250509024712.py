...
def rank_biserial(
    ...
    """
    Rank‐biserial correlation for two independent groups coded 0/1 in labels.
    """
    # assign competition ranks 1..n
    ranks = jnp.argsort(jnp.argsort(measure)) + 1
    # ranks of group 1
    r1 = ranks[labels == 1]
    n1 = jnp.sum(labels == 1)
    # U statistic for group1
    U  = jnp.sum(r1) - n1 * (n1 + 1) / 2.0
    n0 = jnp.sum(labels == 0)
    # r_rb = 1 – 2U/(n1·n0)
