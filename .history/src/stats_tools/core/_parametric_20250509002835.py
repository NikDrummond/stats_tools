...
def _t_test(
    ...
    """
    Two‐sample t‐test (independent groups) without non‐concrete indexing
    or Python‐side branching on traced booleans.
    ...
    # Pooled‐variance standard error & df
    pooled_var = ((n0 - 1) * var0 + (n1 - 1) * var1) / (n0 + n1 - 2)
    se_pooled = jnp.sqrt(pooled_var * (1.0 / n0 + 1.0 / n1))
    # cast df to float32 so it matches the Welch‐df branch
+    df_pooled = jnp.array(n0 + n1 - 2, dtype=measure.dtype)
    ...
    # Choose SE and df via lax.cond on the (possibly traced) equal_var flag
    se, df = lax.cond(
        equal_var,
