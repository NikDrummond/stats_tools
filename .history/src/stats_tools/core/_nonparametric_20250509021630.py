...
def _rank_data(data: jnp.ndarray) -> jnp.ndarray:
    """
    Assign ranks to the data with average ties.

    :param data: 1D JAX array of observations.
    :return: 1D JAX array of float ranks (average for ties).
    """
    # For each element, count how many are less, and how many equal
    # less_i = Σ_j 1[data_i > data_j]
    # eq_i   = Σ_j 1[data_i == data_j]
    d = data
    less = jnp.sum(d[:, None] > d[None, :], axis=1)
    eq   = jnp.sum(d[:, None] == d[None, :], axis=1)
    # average‐tie rank: less + (eq + 1) / 2
