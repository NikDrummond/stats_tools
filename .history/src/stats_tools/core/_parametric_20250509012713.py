...
# src/stats_tools/core/_parametric.py
...
def _one_way_anova(
    ...
    """
    One‐way ANOVA (jitted, no dynamic unique).

    :param labels: 1D integer codes in {0, 1, …, k-1}.
    :param measure: 1D observations of same length.
    :return: (F_statistic, p_value) where p_value is upper‐tail of F(df_between, df_within).
    """
    n = measure.size
    # number of groups = max(label) + 1
    k = jnp.max(labels).astype(int) + 1

    # grand mean and total sum‐of‐squares
    grand_mean = jnp.mean(measure)
    ss_total = jnp.sum((measure - grand_mean) ** 2)

    # between‐groups SS via a lax.fori_loop
    def ssb_body(i, acc):
        mask = labels == i
        n_i = jnp.sum(mask)
    ...
    ss_between = lax.fori_loop(0, k, ssb_body, 0.0)
    ...
    # compute upper‐tail p‐value via regularized incomplete beta:
    #   CDF_F(x; d1, d2) = I_{d1 x/(d1 x + d2)}(d1/2, d2/2)
    z   = (df_between * F_stat) / (df_between * F_stat + df_within)
