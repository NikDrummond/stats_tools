...
def _t_test(
    ...
    """
    Two‐sample t‐test (independent groups) without direct boolean indexing.

    :param measure: 1D array of all observations.
    :param labels: 1D integer array of same length, with values 0 or 1 indicating group.
    :param equal_var: assume equal variances if True (Student’s t), else Welch’s t.
    ...
    if equal_var:
        ...
        se = jnp.sqrt(pooled_var * (1.0 / n0 + 1.0 / n1))
        df = n0 + n1 - 2
    else:
        se = jnp.sqrt(var0 / n0 + var1 / n1)
    ...
    t_stat = diff / se
    p_val = 2.0 * (1.0 - student_t.cdf(jnp.abs(t_stat), df))
