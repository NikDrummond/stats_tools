...
def compare_groups(
    ...
    if method == "parametric":
        if k == 2:
            # leave the kernel to split internally: pass full data + labels
# src/stats_tools/api.py (excerpt)
            stat_jax, pval_jax = _t_test(data_jax, groups_jax, equal_var=not robust)
            stat, p_val = float(stat_jax), float(pval_jax)

            # still extract for effect‐size
            x1 = data_jax[groups_jax == 0]
            x2 = data_jax[groups_jax == 1]
            eff = robust_cohens_d(x1, x2) if robust else cohens_d(x1, x2)
        else:
